...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--keyword",
        default="genius",
        help="Wake word that activates recording (default: genius)",
    )
    parser.add_argument(
        "--api-key",
        dest="api_key",
    ...
    parser.add_argument(
        "--model", default="gpt-3.5-turbo", help="Chat model to request from OpenAI"
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Maximum number of seconds to record once speech starts",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--once", action="store_true", help="Exit after answering a single question"
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
